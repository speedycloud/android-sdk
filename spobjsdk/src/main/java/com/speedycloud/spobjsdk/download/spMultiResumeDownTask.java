...
import com.speedycloud.spobjsdk.utils.DownHelper;
import com.speedycloud.spobjsdk.utils.SDCardTool;
...
public class spMultiResumeDownTask {
    // 下载完成
     ...
     * 下载过程变化的回调
     */
    private spDownLoadStateListener onDownLoadStateListener;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case DOWNLOADPROCESS:
                    onDownLoadStateListener.OnDownLoadProcessChange(process,percent);
                    break;
            }
        }
    };
    ...
    // 构造器
    public spMultiResumeDownTask(Context context, String bucket,String obj, String accessKey, String secretKey,spDownLoadStateListener onDownLoadStateListener) {
        try {
            this.baseServer = "http://oss-cn-beijing.speedycloud.org";
            this.strBucket = bucket;
            this.strObj = obj;
            this.accessKey = accessKey;
            this.secretKey = secretKey;
            this.fileUrl = baseServer + "/" + strBucket + "/" + strObj;
    ...
    protected void startDownload() {
        ...
        if (threadList.size() == 0) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    fileLength = DownHelper.getFileLength(fileUrl,strBucket,strObj, accessKey,secretKey, context);
                    // 开始下载的回调
    ...
    protected void resumeDownload() {
        downloading = false;
        // 暂停下载的回调
        onDownLoadStateListener.OnDownLoadResume(process, percent);
    }
    ...
    private class DownloadRunable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                newParams = DownHelper.getNewParams(strBucket,strObj,accessKey,secretKey);
...
//                connection.setRequestProperty("User-Agent", "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0; .NET CLR 2.0.50727)");
    ...
    synchronized private void updateProgress(int add) {
        process += add;
        percent = (float)process / fileLength;
